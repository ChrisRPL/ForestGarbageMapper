...
class GarbagePointService extends ChangeNotifier
  ...
  @override
  Future<GarbagePoint> getGarbagePointObject(GarbagePointResponse response, DroneService service) async {
    DocumentSnapshot<Map<String, dynamic>> droneSnap = await FirebaseFirestore.instance.collection("/Drone").doc(response.idDrone).get();
    DroneResponse droneResponse = service.getDroneResponse(droneSnap.data()!);
    Drone drone = service.getDroneObject(droneResponse);

    return GarbagePoint(idGarbagePoint: response.idGarbagePoint, idDroneObject: drone, coords: response.coords, imageUrl: response.imageUrl, time: response.time.toDate());
  }
  ...
  Future<List<GarbagePoint>> getGarbagePoints(DroneService service) async {
    List<DocumentSnapshot<Map<String, dynamic>>> pointsSnaps = (await FirebaseFirestore.instance.collection("/GarbagePoint").get()).docs;
    List<GarbagePoint> garbagePoints = [];

    for (DocumentSnapshot<Map<String, dynamic>> pointSnap in pointsSnaps) {
      GarbagePointResponse response = getGarbagePointResponse(pointSnap.data()!);
      GarbagePoint garbagePoint = await getGarbagePointObject(response, service);
